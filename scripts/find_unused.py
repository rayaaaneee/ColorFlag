...
if __name__ == "__main__":
    ...
    countries_to_add: List[str] = [flag_img for flag_img in flag_images if flag_img not in [flag_json["code"] for flag_json in flags_in_json]]
    ...
    countries_w_out_img: List[str] = [flag["code"] for flag in flags_in_json if flag["code"] not in flag_images]
    ...
    for _, _, files in os.walk(country_maps_images_path):
        for file in files:
            code: str = file.replace(".svg", "")
            maps_images.append(code) if file.endswith('.svg') and not flags_in_json else None

    countries_w_out_map: List[str] = [flag["code"] for flag in flags_in_json if flag["code"] not in maps_images and flag.keys().__contains__("non_contry")]
