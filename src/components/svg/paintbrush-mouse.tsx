...
const PaintbrushMouse = ({ color, initialPosition }: PaintbrushInterface) => {
    ...
    useEffect(() => {
        ...
        if (svgRef.current != null) {
            svgRef.current.style.left = `${initialPosition.x - ((coefficient) * svgRef.current.clientWidth - 3)}px`;
            svgRef.current.style.top = `${initialPosition.y - (svgRef.current.clientHeight / coefficient)}px`;
        }

        const handleMouseMove = (event: MouseEvent) => {
            if (svgRef.current) {
                svgRef.current.style.left = `${event.clientX - ((coefficient) * svgRef.current.clientWidth - 3)}px`;
                svgRef.current.style.top = `${event.clientY - (svgRef.current.clientHeight / coefficient)}px`;
            }
        };
